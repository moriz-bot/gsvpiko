...
COMMAND_GET_LAST_PROTOCOL_ERROR = 0x42
TRIGGER_INDEX = 2


def main() -> None:
    """Deliberately trigger one harmless protocol parameter-address error."""
    parser = argparse.ArgumentParser(
        description=(
            "Deliberately request GetLastProtocolError index 2 to trigger a harmless "
            "PARAMETER_ADDRESS_ERROR, then read GetLastProtocolError(0/1)."
        )
    )
    ...
    parser.add_argument(
        "--device-alias",
        default=None,
        help="Limit the deliberate error to one setup device alias.",
    )
    args = parser.parse_args()
    print_cli_option_header([
        ("--setup <setup>", args.setup),
        ("--connection auto|tcp|serial", args.connection),
        ("--device-alias <alias>", args.device_alias or "all"),
    ])

    setup_config = selected_setup(args)
    print("L deliberate protocol-error trigger")
    print("-----------------------------------")
    print(f"diagnostic_time_utc: {datetime.now(timezone.utc).isoformat()}")
    print(f"setup_name: {setup_config.get('name', '<unnamed>')}")
    print("trigger_command: GetLastProtocolError(0x42), index=2")
    print("expected_response_status: PARAMETER_ADDRESS_ERROR")
    print("")

    for target in iter_device_targets(setup_config):
        if args.device_alias and target.alias != args.device_alias:
            continue
        ...
        try:
            ...
            trigger_response = transact(
                transport,
                COMMAND_GET_LAST_PROTOCOL_ERROR,
                [TRIGGER_INDEX],
                require_ok=False,
            )
            print("trigger_response:")
            print(f"  request_raw_hex: {hex_bytes(trigger_response.request_raw)}")
            print(f"  response_raw_hex: {hex_bytes(trigger_response.response_raw)}")
            print(f"  status: 0x{trigger_response.status:02X} ({trigger_response.status_name})")
            print(f"  payload_hex: {hex_bytes(trigger_response.payload)}")
            for index, label in (
                (0, "synchronous_response_frame_error"),
                (1, "asynchronous_measurement_frame_error"),
            ):
                value, response = read_u32(transport, COMMAND_GET_LAST_PROTOCOL_ERROR, index)
                print(f"after_index_{index}: {label}")
                print(f"  read_command_status: 0x{response.status:02X} ({response.status_name})")
                print(f"  stored_protocol_error_uint32: 0x{value:08X} ({value})")
                print(f"  stored_protocol_error: {protocol_error_name(value)}")
                print(f"  response_raw_hex: {hex_bytes(response.response_raw)}")
        except Exception as error:
