...
def main() -> None:
    """Read GetLastProtocolError(0/1) for all setup devices."""
    parser = argparse.ArgumentParser(
        description="Read only GetLastProtocolError indices 0 and 1."
    )
    ...
    print("L protocol-error diagnostic")
    print("---------------------------")
    print(f"diagnostic_time_utc: {datetime.now(timezone.utc).isoformat()}")
    print(f"setup_name: {setup_config.get('name', '<unnamed>')}")
    print("indices: 0=synchronous_response_frame_error, 1=asynchronous_measurement_frame_error")
    print("")
    ...
    for target in iter_device_targets(setup_config):
        ...
        try:
            ...
            for index, label in (
                ...
                print(f"index_{index}: {label}")
                print(f"  request_raw_hex: {hex_bytes(response.request_raw)}")
