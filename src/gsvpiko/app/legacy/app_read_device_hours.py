...
COMMAND_READ_DEVICE_HOURS = 0x56


def main() -> None:
    """Read device-hour counters for all setup devices."""
    parser = argparse.ArgumentParser(
        description="Read ReadDeviceHours(0/1) for all devices in a setup."
    )
    ...
    for target in iter_device_targets(setup_config):
        ...
        try:
            ...
            for index, label in ((0, "absolute_monotonic"), (1, "settable")):
                hours, response = read_float32(transport, COMMAND_READ_DEVICE_HOURS, index)
                print(f"device_hours_{index}: {label}")
                print(f"  value_h: {hours:.6f}")
