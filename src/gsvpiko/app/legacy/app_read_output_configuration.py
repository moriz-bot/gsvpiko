...
COMMAND_GET_DIO_DIRECTION = 0x59
COMMAND_GET_DIO_TYPE = 0x5B


def main() -> None:
    """Read analog-output and digital-output configuration."""
    parser = argparse.ArgumentParser(
        description=(
            "Read GSV-8 analog-output types and DIO direction/type settings. "
            "This is read-only and intended for error-context diagnostics."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Highest analog-output channel to read. Default: 8.",
    )
    parser.add_argument(
        "--show-dio-types",
        action="store_true",
        help="Also read GetDIOtype for DIO 1-16.",
    )
    ...
    print_cli_option_header([
        ...
        ("--show-dio-types", args.show_dio_types),
    ])

    setup_config = selected_setup(args)
    print("L output-configuration diagnostic")
    print("---------------------------------")
    ...
    for target in iter_device_targets(setup_config):
        ...
        try:
            ...
            for channel in range(1, max(1, args.max_aout_channel) + 1):
                response = transact(
                    transport,
                    COMMAND_GET_AOUT_TYPE,
                    [channel],
                    require_ok=False,
                )
                if not response.ok:
                    print(f"  aout_{channel}: status=0x{response.status:02X} ({response.status_name})")
                    continue
                if len(response.payload) < 2:
                    print(f"  aout_{channel}: unexpected_payload={hex_bytes(response.payload)}")
                    continue
                type_enum = response.payload[0]
                mode = response.payload[1]
                print(
                    f"  aout_{channel}: type={type_enum} ({aout_type_name(type_enum)}), "
                    f"inactive={bool(mode & 0x02)}, direct_mode={bool(mode & 0x01)}, "
                    f"alternate_input={bool(mode & 0x04)}, raw={hex_bytes(response.response_raw)}"
                )

            print("dio_direction_groups:")
            response = transact(transport, COMMAND_GET_DIO_DIRECTION, [0], require_ok=False)
            if not response.ok:
                print(f"  groups: status=0x{response.status:02X} ({response.status_name})")
            elif not response.payload:
                print("  groups: unexpected_empty_payload")
            else:
                value = response.payload[0]
                print(f"  raw_group_bits: 0x{value:02X}; response_raw_hex: {hex_bytes(response.response_raw)}")
                for group in range(1, 5):
                    bit = (value >> (group - 1)) & 1
                    print(f"  group_{group}: {dio_direction_name(bit)}")

            if args.show_dio_types:
                print("dio_types:")
                for dio_no in range(1, 17):
                    response = transact(transport, COMMAND_GET_DIO_TYPE, [dio_no], require_ok=False)
                    if not response.ok:
                        print(f"  dio_{dio_no}: status=0x{response.status:02X} ({response.status_name})")
                        continue
                    if len(response.payload) != 4:
                        print(f"  dio_{dio_no}: unexpected_payload={hex_bytes(response.payload)}")
                        continue
                    dio_type = int.from_bytes(response.payload[:3], byteorder="big", signed=False)
                    assigned_channel = response.payload[3]
                    print(
                        f"  dio_{dio_no}: type_uint24=0x{dio_type:06X}, "
                        f"assigned_channel={assigned_channel}, raw={hex_bytes(response.response_raw)}"
                    )
        except Exception as error:
